...
router.post('/parse', async (req, res) => {
    ...
    try {
        ...
        return res
            .status(500)
            .json({ message: 'Failed to parse invalid PGN.' });
    }
    ...
    for (let pgnMove of parsedPGN.moves) {
        ...
        try {
            ...
            return res
                .status(400)
                .json({ message: 'PGN contains illegal moves.' });
        }
    ...
    if (process.env.RECAPTCHA_SECRET) {
        try {
            ...
            if (!captchaResult.success) {
                return res
                    .status(400)
                    .json({ message: 'You must complete the CAPTCHA.' });
            }
        } catch (err) {
            return res.status(500).json({ message: 'You are a robot!!111!1!' });
        }
    }
